...
var SimpleGraphQL = {
  ...
  makeMergedSchema: function makeMergedSchema(schema, mergeInfo, context) {
    //invariant(mergeInfo.linkInfo, 'Must provide linkinfo')
    console.log('makeMergedSchema');

    var buildLinkInfos = function buildLinkInfos(schema, schemaMerged, linkInfos, context) {
      ...
      _lodash2.default.forOwn(linkInfos, function (ext, schemaName) {
        if (ext.fields) {
          ...
          _lodash2.default.forOwn(ext.fields, function (field, fieldName) {
            ...
            var linkIdName = _StringHelper2.default.toInitialLowerCase(fieldName) + 'Id';
            (0, _invariant2.default)(schema.getType(schemaName).getFields()[linkIdName], 'Must provide valid schema: ' + schemaName + '[' + linkIdName + ']');

            (0, _invariant2.default)(schemaMerged.getType(fieldName).getFields()['id'], 'Must provide valid merged schema: ' + fieldName + '[id]');
            ...
            resolvers[schemaName][fieldName] = {
              ...
              resolve: function resolve(root, args, contxt, info) {
                console.log('origin remote field resolve', root);

                var defaultResolve = function defaultResolve(root, args, context, info, sgContext) {
                  var globalId = relay.toGlobalId(_StringHelper2.default.toInitialUpperCase(fieldName), root[linkIdName]);
                  console.log('default remote field resolve', root, globalId);
                  return info.mergeInfo.delegateToSchema({
                    schema: schemaMerged,
                    operation: 'query',
                    fieldName: _StringHelper2.default.toInitialLowerCase(fieldName),
                    args: {
                      id: globalId
                    },
                    context: context,
                ...
                var fn = context.wrapFieldResolve({
          ...
          if (!_lodash2.default.isEmpty(typeDef)) {
            gqls.push((0, _dedent2.default)(_templateObject, schemaName, typeDef));
          }
        }
                ...
                return regeneratorRuntime.wrap(function _callee$(_context) {
                  while (1) {
                    switch (_context.prev = _context.next) {
                      case 0:
                        return _context.abrupt('return', value.resolve(args, context, info, context.getSGContext()));
                ...
                return regeneratorRuntime.wrap(function _callee2$(_context2) {
                  while (1) {
                    switch (_context2.prev = _context2.next) {
                      case 0:
                        return _context2.abrupt('return', value.resolve(args, context, info, context.getSGContext()));
      ...
      if (queryDefs && queryDefs.length) {
        gqls.push((0, _dedent2.default)(_templateObject2, queryDefs));
      }

      if (mutationDefs && mutationDefs.length) {
        gqls.push((0, _dedent2.default)(_templateObject3, mutationDefs));
      }
    ...
    var _buildLinkInfos = buildLinkInfos(schema, mergeInfo.schemaMerged, mergeInfo.linkInfo, context),
        gqls = _buildLinkInfos.gqls,
        resolvers = _buildLinkInfos.resolvers;

    var schemas = [schema, mergeInfo.schemaMerged];
    if (gqls) {
      var gql = gqls.join('\n');
  ...
  build: function build(args) {
    ...
    finalQueries['node'] = {
      ...
      resolve: context.wrapQueryResolve({
        ...
        $type: context.nodeInterface,
        resolve: function () {
          var _ref3 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3(args, context, info, sgContext, invoker) {
            var id, record;
              ...
              while (1) {
                switch (_context3.prev = _context3.next) {
                  case 0:
                    ...
                    return _context3.abrupt('return', finalQueries['viewer'].resolve(null, args, context, info));
